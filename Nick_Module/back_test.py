...
def backtest_macd_strategy_52high(df, initial_cash=10000):
    """
    Backtests a simple MACD strategy where you buy when MACD > 0 and sell when MACD < 0.
    Records the balance, cash, and position over time in new columns.
    ...
    for i, row in df.iterrows():
        ...
        if df['MACD'].iloc[i] > 0 and df['MACD'].iloc[i-1] <= 0 and row['Close'] >= max_price * 0.7:
            # Buy signal
            if cash > 0:
                position = cash // df['Close'].iloc[i]
                cash = cash - position * df['Close'].iloc[i]
        elif df['MACD'].iloc[i] < 0 and df['MACD'].iloc[i-1] >= 0 or row['Close'] <= max_price * 0.7:
            # Sell signal
            if position > 0:
                cash = cash + position * df['Close'].iloc[i]
                position = 0
