...
def clean_quarterly_fundamental(Ticker:str, current_month:str=CURRENT_MONTH, n:int=-4)->pd.DataFrame:
    ...
    merge_df['EPS_YOY_pct_change'] = ((merge_df['reportedEPS'] - merge_df['reportedEPS'].shift(n)) / merge_df['reportedEPS'].shift(n)).replace(0, 1)
    # add percentage change of revenue
    merge_df['revenue_YOY_pct_change'] = ((merge_df['totalRevenue'] - merge_df['totalRevenue'].shift(n)) / merge_df['totalRevenue'].shift(n)).replace(0, 1)
    # add percentage change of net income
    merge_df['netIncome_YOY_pct_change'] = ((merge_df['netIncome'] - merge_df['netIncome'].shift(n)) / merge_df['netIncome'].shift(n)).replace(0, 1)
    ...
    merge_df['netProfitMargin_YOY_pct_change'] = ((merge_df['netProfitMargin'] - merge_df['netProfitMargin'].shift(n)) / merge_df['netProfitMargin'].shift(n)).replace(0, 1)
...
#example
# example = generate_fundamental('TXRH')
# x = example[1]
# x